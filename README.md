...
***Поддерживаемые types***: ```map```, ```plugin```, ```module```, ```uploader```, ```bundle```;
...
1) При удалении ресурса также будут удалены все бандлы, его содержащие
2) Если версия плагина не указана (т.е. не заканчивается на %номер_версии), то будут удалены все версии данного плагина и соответственно будут удалены все бандлы их содержащие
3) Право на удаление есть только у главного администратора.
4) Также через этот запрос можно лишить пользователя права на загрузку данных в репозиторий
