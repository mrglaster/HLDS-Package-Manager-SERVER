...
1) Замена Apache FTP Server на что-то другое
2) Внедрение поддержки выбора протокола передачи файлов (FTP/HTTP)
3) Выбор: HTTP/HTTPS
4) Отказ от передачи файлов как base64 строки полем JSON; заменить на Multipart
5) Реализовать загрузку бандлов через манифест
6) Внедрение поддержки других СУБД
